...
❌ **禁止**产出只罗列文件名和大小的"项目概述"。这类文章对 agent 没有实际价值。

✅ 每篇文章必须通过**"镜像地球测试"**：一个从未见过原始材料的 agent，仅凭 wiki 文章能否理解该系统的：
- 背景和目的
- 硬件/软件架构
- 各模块功能设计
- 交互流程或业务动线
...
| 维度 | 最低要求 |
|------|---------|
| 篇幅 | 每个主题至少 1 篇 ≥2KB 的主文章 |
| 内容 | 必须是理解重组后的结构化知识，不是原文摘录 |
| 溯源 | sources 字段标注所有被融合的源文件路径 |
| 交叉引用 | 至少引用 1 个相关主题 |

### Phase 2 提取 ≠ Phase 3 完成

⚠️ 二进制文件（docx/PDF/pptx/xmind）提取为文本只是 Phase 2 的输出。**不能**直接把提取件放入 wiki——必须经过 Phase 3 理解融合。

提取件应放入 `<RAW_DIR>/_extracted/`（原始材料目录），融合完成后移入归档。

### 常见处理错误

| 错误 | 表现 | 修正 |
|------|------|------|
| 文件清单冒充文章 | "项目概述"只有文件名+大小 | 读取提取件内容，写出真实知识 |
| 提取件留在 wiki | `_原始提取/` 混在 wiki 目录中 | 融合后移回原始材料目录 |
| 提取代替融合 | 以为 `extracted:` 标记的文件就是完成品 | 提取只是 Phase 2 输出，必须走完 Phase 3 |
| Confluence 子页面遗漏 | 只匹配 `data-page-id` 属性 | 同时匹配 `href="/pages/viewpage.action?pageId=XXXX"` — 折叠页用 href 暴露 |
| PPT 内容被忽略 | 只提取了 docx/pdf | 同时提取 pptx（python-pptx 或 markitdown） |

### 二进制提取工具链

| 格式 | 工具 | 备注 |
|------|------|------|
| .docx | python-docx / markitdown | 大文件限制段落数 |
| .pdf | PyPDF2 / markitdown | 大文件限制页数（前 50 页） |
| .pptx | python-pptx / markitdown | 从 slides 提取文本+表格 |
| .xmind | zipfile + json | 解析 content.json 递归提取标题 |
| .html | defuddle | 网页→干净 Markdown |

编码注意：curl 用 `-o` 写文件，禁止通过管道（破坏 UTF-8）；文本过滤 surrogate 字符。

---

## Phase 4 增强：Obsidian 多视图

### Base 文件设计原则

`.base` 数据库视图的列设计应从实际浏览需求出发：

- **所属项目** — 从文件路径提取
- **涉及实体** — 从项目映射到核心系统（如 DSMS系统、IVP系统）
- **文档类型** — 从文件名推断（需求文档/接口规格/状态机/测试文档/设计文档/增量需求/管理功能/监控/统计/日志审计/索引/通用）
- **来源类型** — 融合文章 vs 规格文档 vs 二进制提取
- **知识密度** — 简略(<1KB) / 中等(1-5KB) / 详细(>5KB)
- **用户角色** — 标注用户个人负责的项目

推荐的视图分组：全部文档、核心系统、项目文档、用户负责、详细规格、卡片浏览。

### Canvas 设计原则

`.canvas` 画布按"核心系统在上，项目在下"的布局，用 group 分区，edge 标注依赖关系。颜色区分：🔴 用户核心工作、🟠 关联项目、🟡 其他项目。
...
本流水线处理过的最复杂项目群（21 个项目，8000+ 原始文件）呈现以下规律：

- **核心系统（01-05）**：如果有 agent 前期撰写的产品规格 MD，可直接摄入 wiki，达到 95% 知识密度
- **项目（06-21）**：知识密度取决于原始材料质量。二进制文件为主的项目需要通过提取+融合补足，但最终密度受限于原始材料本身的丰富程度
- **自知局限**：原始材料只有 2-3 个 PDF 的项目（如小型卫星方案），无法凭空创造知识。这类项目在 wiki 中的覆盖度必然有限
...
新一批原始材料到达
  ↓
Phase 1: skill-manager → knowledge-cleanup（查重去版本）
  ↓
Phase 2: 判断材料格式
  ├── MD 文件（agent 撰写）→ 直接摄入 wiki
  ├── 网页 → defuddle → MD
  ├── docx/PDF/pptx/xmind → markitdown 批量转换 → 文本提取
  └── 图片/音频 → 跳过或 describe_image
  ↓
Phase 3: 逐文件阅读 → 理解 → 逐主题融合撰写
  ├── 每完成一个主题 → 移出提取件
  └── 全部完成 → 更新 index.md + log.md
  ↓
Phase 4: 创建 .base + .canvas → frontmatter 增强
  ↓
Phase 5: 报告密度分布 → 识别薄弱项目 → 用户决定是否补充
