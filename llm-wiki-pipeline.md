...
description: LLM WIKI 知识工厂 — 端到端知识库构建流水线：查重清理 → 获取与转换 → 初步融合(3A) → 深度知识(3B,70%密度) → Obsidian最佳范式输出 → 持续维护。整合 knowledge-cleanup + everything-markdown + defuddle + karpathy-llm-wiki + multi-agent-discuss + obsidian-*
---
...
> 适用于快速覆盖多项目、建立全局索引。产物：每个项目 1 篇"项目概述.md"。

⚠️ **注意**：初步融合只是 Phase 3 的起点，不是终点。项目概述本质是"目录 + 结构索引"，知识密度约 15-20%。

### Phase 3B: 深度知识层（70% 密度融合）⭐ 核心方法论

> **这是 Phase 3 的真正目标状态**。当用户需要深度理解一个系统时，必须走 Phase 3B。
...
| **70%（知识层）** | 包含设计决策因果、数值参数、端到端流程、接口契约 | "为什么用四层实体模型而不是两层" |

**核心认知**：70% 密度不是"压缩掉 30% 的字数"，而是"保留 70% 的非冗余知识"。原始材料中大量的模板套话、导航段落、重复描述被去掉后，剩下的知识以结构化方式重新表达。

#### 深度知识的硬指标（S/A/B/C 四级）

| 级别               | 适用对象          | 硬指标                                                             |
| ---------------- | ------------- | --------------------------------------------------------------- |
| **S 级**（核心技术系统）  | 用户直接负责的核心业务系统 | ①≥3 设计决策因果分析 ②≥5 具体数值/参数 ③≥1 端到端业务流程（含异常分支） ④≥2 系统接口契约 ⑤≥3000 字 |
| **A 级**（项目/业务系统） | 重要项目或业务系统     | 前 4 项硬指标                                                        |
| **B 级**（产品/方案类）  | 产品原型、数据网关等    | ≥2 设计决策 + ≥1 业务流程 + ≥1500 字                                     |
| **C 级**（小型/支撑项目） | 小型方案、外围支撑项目等  | ≥1 设计决策 + ≥1500 字                                               |

#### 粒度原则：一个子系统 ≠ 一篇文章

**❌ 错误**：整个系统写一篇 3000 字文章 → 每个子系统分 200 字 → 退回命名层

**✅ 正确**：按子系统粒度产出：
```
<系统>-综合/
  <系统>知识全景.md            ← 5000-8000 字（顶层设计决策 + 架构 + 全流程 + 接口全景）
<系统>-核心模块A/
  <核心模块A>深度知识.md       ← 3000-5000 字
<系统>-核心模块B/
  <核心模块B>深度知识.md       ← 3000-5000 字
<系统>-调度引擎/
  <调度引擎>深度知识.md        ← 2000-4000 字
<系统>-外部接口/
  <系统>关键外部接口深度知识.md ← 5000-8000 字
```

**产出估算**：一个 S 级系统（含 50-70 个源文件）→ 5-6 篇文章 → 2-3 万字。

#### 双层结构：补充不替换

深度文章是**"理解层"**，原始规格文档是**"参考层"**：
...
<模块目录>/
  ...
  监控面板.md                  ← 保留（参考层）
  README.md                    ← 保留
  <模块>深度知识.md            ← 新增（LLM 融合理解，理解层）
```

**原则**：参考层用于查具体参数，理解层用于理解系统全貌。两层互不覆盖，通过 wikilink 关联。

#### 多 Agent 圆桌讨论：设计方法论的前置步骤

在对一个复杂系统启动深度知识写作前，应先通过 `multi-agent-discuss` Skill 组织圆桌讨论：

**推荐角色配置**：
- **知识架构师** — 定义"70% 密度"在该项目中的具体标准，设计文章结构
- **领域专家** — 判断知识价值层级（哪些是核心，哪些可简化）
- **执行专家** — 设计实操流程，评估工作量和粒度
- **质量门神** — 定义可验证的交付标准，防止滑回概述

**讨论产出**：分级方案 + 文章粒度 + 预估工作量 + 检查清单。
...
- ❌ 出现"详见原始文档"（说明没提取）
- ❌ 连续 3 段以上只是列表/目录结构
- ❌ 无法回答"如果 X 变了，Y 会怎样"类因果问题
- ❌ 一个含 50+ 源文件的系统只写了一篇 <5000 字的文章
...
| 维度 | 初步融合（3A） | 深度知识（3B） |
|------|-------------|-------------|
| 篇幅 | 每个主题 ≥800 字 | S 级系统 2-3 万字（5-6 篇） |
| 内容 | 结构索引 + 概述 | 设计决策因果 + 数值参数 + 业务流程 + 接口契约 |
| 溯源 | sources 字段 | sources + 原文 wikilink 关联 |
| 交叉引用 | ≥1 个相关主题 | ≥3 个相关系统/项目 |
| 密度 | 15-40% | **70%** |
...
| 文档平台子页面遗漏 | 只匹配展开节点的属性      | 同时匹配折叠节点的 href 链接模式 |
...
本流水线处理过的最复杂项目群（20+ 个项目，数千原始文件）呈现以下规律：

- **核心系统**：如果有 agent 前期撰写的产品规格 MD，可直接摄入 wiki，达到 95% 知识密度
- **外围项目**：知识密度取决于原始材料质量。二进制文件为主的项目需要通过提取+融合补足
- **自知局限**：原始材料只有 2-3 个文档的项目，无法凭空创造知识。这类项目在 wiki 中的覆盖度必然有限
